...
## [Agent: Claude Code] 2026-06-11 — Structured implementation plan (fixes + new features, all surfaces)

**Objective (user request):** "Make a plan structured ... current fix as well as new implement on all ends frontend backed desktop ... covers in plan needed for actualy creating all this actualy working and high quality and use any skill needed or create new." Builds directly on the prior session's market-research report and a (text-only, no-tool-call) "what does the system lack" breakdown given to the user, in which the user confirmed: **web app focus = live in-person/face-to-face negotiation**, **desktop app focus = virtual meetings** (matches existing `SourceMode.IN_PERSON_WEB` vs `SourceMode.VIRTUAL_COMPANION_DESKTOP` in `backend/app/models/companion.py` — no architectural pivot needed, just build-out direction).

**Deliverable (committed this session):** `docs/plans/2026-06-11-implementation-plan/` — 6 files, written across separate Write calls (per the standing 32k-output-token avoidance rule):
- `00_overview.md` — phasing/dependency graph, cross-cutting execution rules, skill strategy summary.
- `01_phase0_foundations.md` — BLOCKING items: 0.1 restore `frontend/lib/types.ts` (build-breaking, exact inferred export shapes from code map); 0.2 billing/metering/entitlements infra (Stripe — flagged for AskUserQuestion before build, biggest single item, gates all monetization); 0.3 geo-aware pre-session consent (extends existing `PRIVACY_CONSENT_GRANTED`/`ConsentPayload`/`consent_mode`); 0.4 latency instrumentation (new trace events in `session_trace.py` for the <1.5s ask budget, feeds the new e2e skill); 0.5 vision-dominance bug fix (prompt/coalescing fix in `_send_coalesced_intel`/`ai_assets.py`, told to re-check HANDOFF history first — see the truncation/vision-precedence entry above, already partially addressed); 0.6 ask-transcription truncation (the `companion_runtime.py:369` "multi-writer race at hold-release" — also already partially addressed by the `ASK_AI_ACTIVITY_END_DELAY_SECONDS` fix above; this item should re-validate whether that fix was sufficient or whether the lock needs to extend to the hold-release buffer-close path).
- `02_phase1_web_inperson.md` — 1.1 mobile PWA (manifest/wake-lock/background-audio/responsive layout — confirmed no manifest.json exists yet); 1.2 glanceable cue-card format (new structured `CUE_CARD`/`AI_RESPONSE` payload + prompt change, gated by new `CUE_CARD_FORMAT_ENABLED` flag, shared with desktop 2.3); 1.3 room-audio robustness (speaker-confidence-collapse → `DEGRADED_MODE_UPDATE` → manual speaker-override prompt, room-mode VAD/gain preset); 1.4 pre-meeting prep wizard (target/walkaway/BATNA input, ties to Event Pass tier, flags `services/market_research.py` as a possible revival candidate — coordinate with Phase 4 dead-code removal so it isn't deleted out from under this); 1.5 in-person verbal consent disclosure (depends on 0.3).
- `03_phase2_desktop_virtual.md` — 2.1 macOS capture support (system audio via ScreenCaptureKit-era APIs, mic-isolation v1/v2 split, references the approved-but-unimplemented `docs/DRIVERLESS_MIC_ISOLATION_PLAN.md` as design precedent); 2.2 auto meeting detection (v1 = window-title polling reusing `inferPlatform`, v2 = calendar OAuth); 2.3 cue-card overlay mode (new `applyOverlayPresentation` mode, consumes 1.2's backend payload); 2.4 post-call debrief surface (productizes existing `session_trace`/`report.md`, currently generated but never shown in any UI).
- `04_phase3_shared_artifacts_growth.md` — 3.1 concession tracker (audit `strategy_history` first — may be frontend-only); 3.2 BATNA/walkaway memory panel (distinguishes user's own BATNA from AI-inferred `walkaway_threshold`); 3.3 offer-history timeline (report.md + debrief); 3.4 "terms agreed so far" panel (new `TERMS_STATUS_UPDATE`, prompt change, flag `TERMS_TRACKING_ENABLED`); 3.5 debrief/share page with token-gated public link (growth loop, "desktop captures, web shares"); 3.6 HubSpot CRM sync for Team tier (explicitly sequenced last — depends on 0.2 billing entitlements existing first).
- `05_skills_and_verification.md` — per-item skill mapping table; proposes a NEW skill `negotiation-session-e2e` (wraps the existing `backend/evals/` harness + `EVAL_MODE_ENABLED` + `audio_fixtures/manifest.json`, asserts report.md generation, latency budget, schema conformance for new payload types) to be built in Phase 0 alongside 0.4 as a regression guard for everything after; full acceptance-criteria checklist per phase; recommends starting next session with 0.1 + 0.4 + the new skill.

**Cross-references resolved during planning (read live, not from stale docs):** confirmed via fresh greps that `frontend/lib/types.ts` is still missing, no `manifest.json`/service worker exists in `frontend/`, `StrategyUpdate`/`OutcomeSummary` message shapes (`messages.py:108,121`) already cover most of the concession-tracker/debrief data needs, and `backend/evals/` + `EVAL_MODE_ENABLED` + `audio_fixtures/` already provide the foundation for the proposed e2e skill.

**Verification status:** plan written and is believed structurally sound (cites real path:line references throughout); NO CODE CHANGED this session — this is planning only. Not yet committed at the time of writing this entry.

**Next action for incoming agent:** commit+push this plan to `claude/code-analysis-documentation-ejrtef`. Then, per the plan's own recommendation, start with Phase 0 item 0.1 (`frontend/lib/types.ts` restoration — quick, unblocks frontend build) and/or 0.4 + the `negotiation-session-e2e` skill (establishes regression-testing foundation). Do NOT start 0.2 (billing) without first running `AskUserQuestion` on payment processor choice (Stripe assumed) and tier shape, per the plan's explicit flag. Before touching 0.5/0.6 (vision-dominance / ask-transcription truncation), re-read the immediately-preceding HANDOFF entry above (2026-06-11, ASK_AI truncation/vision-precedence fixes) since those items may already be partially resolved — confirm via a NEW session (system instruction is session-start-applied) before concluding more work is needed.
